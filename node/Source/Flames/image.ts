...
import { ColorPalette, colorFromPalette, defaultPalette, getRandomColorPalette } from "../palette"
...
export function createDensityArray(resolution: XY, flames: Flames, densityFactor: number): number[] {
...
export function createFlamesPixelBufferFromDensity(resolution: XY, heatmap: number[], p: ColorPalette, densityFactor: number): Uint16Array {
...
export async function createFlameImage(resolution: XY, flames: Flames, outfile = "output.png", supersample = false, log = false) {
	...
	let pixels = createFlamesPixelBufferFromDensity(sampleResolution, heatMap, palette, densityFactor)
