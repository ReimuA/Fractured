...
}`,le=`@group(0) @binding(0) var<storage, read_write> heatmap: array<u32>;\r
@group(0) @binding(1) var<storage, read_write> pixels: array<u32>;\r
...
}  `,$e=`@group(0) @binding(0) var<storage, read_write> heatmap: array<u32>;\r
@group(0) @binding(1) var<storage, read_write> pixels: array<u32>;\r
@group(0) @binding(2) var<storage, read_write> image: array<u32>;\r
@group(0) @binding(3) var<storage, read_write> heatmapMax: u32;\r
@group(0) @binding(4) var<storage, read_write> blurredImage: array<u32>;\r
...
}  `,Je=`@group(0) @binding(0) var<storage, read_write> heatmap: array<u32>;\r
@group(0) @binding(1) var<storage, read_write> pixels: array<u32>;\r
@group(0) @binding(2) var<storage, read_write> output: array<u32>;\r
@group(0) @binding(3) var<storage, read_write> heatmapMax: u32;\r
\r
...
}`,Ze=`@group(0) @binding(0) var<storage, read_write> heatmap: array<atomic<u32>>;\r
@group(0) @binding(1) var<storage, read_write> pixels: array<u32>;\r
@group(0) @binding(2) var<storage, read_write> image: array<u32>;\r
...
}  `,Qe=`@group(0) @binding(0) var<storage, read_write> heatmap: array<u32>;\r
@group(0) @binding(1) var<storage, read_write> pixels: array<u32>;\r
@group(0) @binding(2) var<storage, read_write> image: array<u32>;\r
@group(0) @binding(3) var<storage, read_write> heatmapMax: u32;\r
@group(0) @binding(4) var<storage, read_write> blurredImage: array<u32>;\r
...
`)this._line++;else if(e=="*"){if(this._peekAhead()=="/"&&(this._advance(),n--,n==0))return!0}else e=="/"&&this._peekAhead()=="*"&&(this._advance(),n++)}return!0}}let t=a.none;for(;;){let n=this._findType(e);const s=this._peekAhead();if(e==">"&&(s==">"||s=="=")){let i=!1,c=this._tokens.length-1;for(let h=0;h<5&&c>=0;++h,--c)if(this._tokens[c].type===a.tokens.less_than){c>0&&this._tokens[c-1].isArrayOrTemplateType()&&(i=!0);break}if(i)return this._addToken(n),!0}if(n===a.none){let i=e,c=0;const h=2;for(let f=0;f<h;++f)if(i+=this._peekAhead(f),n=this._findType(i),n!==a.none){c=f;break}if(n===a.none)return t===a.none?!1:(this._current--,this._addToken(t),!0);e=i,this._current+=c+1}if(t=n,this._isAtEnd())break;e+=this._advance()}return t===a.none?!1:(this._addToken(t),!0)}_findType(e){for(const t in a.keywords){const n=a.keywords[t];if(this._match(e,n.rule))return n}for(const t in a.tokens){const n=a.tokens[t];if(this._match(e,n.rule))return n}return a.none}_match(e,t){if(typeof t=="string"){if(t==e)return!0}else{const n=t.exec(e);if(n&&n.index==0&&n[0]==e)return!0}return!1}_isAtEnd(){return this._current>=this._source.length}_isWhitespace(e){return e==" "||e=="	"||e=="\r"}_advance(e=0){let t=this._source[this._current];return e=e||0,e++,this._current+=e,t}_peekAhead(e=0){return e=e||0,this._current+e>=this._source.length?"\0":this._source[this._current+e]}_addToken(e){const t=this._source.substring(this._start,this._current);this._tokens.push(new ze(e,t,this._line))}}class Ct{constructor(){this._tokens=[],this._current=0,this._context=new ct}parse(e){this._initialize(e);let t=[];for(;!this._isAtEnd();){const n=this._global_decl_or_directive();if(!n)break;t.push(n)}return t}_initialize(e){if(e)if(typeof e=="string"){const t=new Ut(e);this._tokens=t.scanTokens()}else this._tokens=e;else this._tokens=[];this._current=0}_error(e,t){return console.error(e,t),{token:e,message:t,toString:function(){return`${t}`}}}_isAtEnd(){return this._current>=this._tokens.length||this._peek().type==a.eof}_match(e){if(e instanceof u)return this._check(e)?(this._advance(),!0):!1;for(let t=0,n=e.length;t<n;++t){const s=e[t];if(this._check(s))return this._advance(),!0}return!1}_consume(e,t){if(this._check(e))return this._advance();throw this._error(this._peek(),t)}_check(e){if(this._isAtEnd())return!1;const t=this._peek();if(e instanceof Array){let n=t.type;return e.indexOf(n)!=-1}return t.type==e}_advance(){return this._isAtEnd()||this._current++,this._previous()}_peek(){return this._tokens[this._current]}_previous(){return this._tokens[this._current-1]}_global_decl_or_directive(){for(;this._match(a.tokens.semicolon)&&!this._isAtEnd(););if(this._match(a.keywords.alias)){const t=this._type_alias();return this._consume(a.tokens.semicolon,"Expected ';'"),t}if(this._match(a.keywords.enable)){const t=this._enable_directive();return this._consume(a.tokens.semicolon,"Expected ';'"),t}const e=this._attribute();if(this._check(a.keywords.var)){const t=this._global_variable_decl();return t!=null&&(t.attributes=e),this._consume(a.tokens.semicolon,"Expected ';'."),t}if(this._check(a.keywords.override)){const t=this._override_variable_decl();return t!=null&&(t.attributes=e),this._consume(a.tokens.semicolon,"Expected ';'."),t}if(this._check(a.keywords.let)){const t=this._global_let_decl();return t!=null&&(t.attributes=e),this._consume(a.tokens.semicolon,"Expected ';'."),t}if(this._check(a.keywords.const)){const t=this._global_const_decl();return t!=null&&(t.attributes=e),this._consume(a.tokens.semicolon,"Expected ';'."),t}if(this._check(a.keywords.struct)){const t=this._struct_decl();return t!=null&&(t.attributes=e),t}if(this._check(a.keywords.fn)){const t=this._function_decl();return t!=null&&(t.attributes=e),t}return null}_function_decl(){if(!this._match(a.keywords.fn))return null;const e=this._consume(a.tokens.ident,"Expected function name.").toString();this._consume(a.tokens.paren_left,"Expected '(' for function arguments.");const t=[];if(!this._check(a.tokens.paren_right))do{if(this._check(a.tokens.paren_right))break;const i=this._attribute(),c=this._consume(a.tokens.ident,"Expected argument name.").toString();this._consume(a.tokens.colon,"Expected ':' for argument type.");const h=this._attribute(),f=this._type_decl();f!=null&&(f.attributes=h,t.push(new Mt(c,f,i)))}while(this._match(a.tokens.comma));this._consume(a.tokens.paren_right,"Expected ')' after function arguments.");let n=null;if(this._match(a.tokens.arrow)){const i=this._attribute();n=this._type_decl(),n!=null&&(n.attributes=i)}const s=this._compound_statement();return new ee(e,t,n,s)}_compound_statement(){const e=[];for(this._consume(a.tokens.brace_left,"Expected '{' for block.");!this._check(a.tokens.brace_right);){const t=this._statement();t!==null&&e.push(t)}return this._consume(a.tokens.brace_right,"Expected '}' for block."),e}_statement(){for(;this._match(a.tokens.semicolon)&&!this._isAtEnd(););if(this._check(a.keywords.if))return this._if_statement();if(this._check(a.keywords.switch))return this._switch_statement();if(this._check(a.keywords.loop))return this._loop_statement();if(this._check(a.keywords.for))return this._for_statement();if(this._check(a.keywords.while))return this._while_statement();if(this._check(a.keywords.continuing))return this._continuing_statement();if(this._check(a.keywords.static_assert))return this._static_assert_statement();if(this._check(a.tokens.brace_left))return this._compound_statement();let e=null;return this._check(a.keywords.return)?e=this._return_statement():this._check([a.keywords.var,a.keywords.let,a.keywords.const])?e=this._variable_statement():this._match(a.keywords.discard)?e=new kt:this._match(a.keywords.break)?e=new bt:this._match(a.keywords.continue)?e=new St:e=this._increment_decrement_statement()||this._func_call_statement()||this._assignment_statement(),e!=null&&this._consume(a.tokens.semicolon,"Expected ';' after statement."),e}_static_assert_statement(){if(!this._match(a.keywords.static_assert))return null;let e=this._optional_paren_expression();return new lt(e)}_while_statement(){if(!this._match(a.keywords.while))return null;let e=this._optional_paren_expression();const t=this._compound_statement();return new ht(e,t)}_continuing_statement(){if(!this._match(a.keywords.continuing))return null;const e=this._compound_statement();return new ft(e)}_for_statement(){if(!this._match(a.keywords.for))return null;this._consume(a.tokens.paren_left,"Expected '('.");const e=this._check(a.tokens.semicolon)?null:this._for_init();this._consume(a.tokens.semicolon,"Expected ';'.");const t=this._check(a.tokens.semicolon)?null:this._short_circuit_or_expression();this._consume(a.tokens.semicolon,"Expected ';'.");const n=this._check(a.tokens.paren_right)?null:this._for_increment();this._consume(a.tokens.paren_right,"Expected ')'.");const s=this._compound_statement();return new pt(e,t,n,s)}_for_init(){return this._variable_statement()||this._func_call_statement()||this._assignment_statement()}_for_increment(){return this._func_call_statement()||this._increment_decrement_statement()||this._assignment_statement()}_variable_statement(){if(this._check(a.keywords.var)){const e=this._variable_decl();if(e===null)throw this._error(this._peek(),"Variable declaration expected.");let t=null;return this._match(a.tokens.equal)&&(t=this._short_circuit_or_expression()),new I(e.name,e.type,e.storage,e.access,t)}if(this._match(a.keywords.let)){const e=this._consume(a.tokens.ident,"Expected name for let.").toString();let t=null;if(this._match(a.tokens.colon)){const s=this._attribute();t=this._type_decl(),t!=null&&(t.attributes=s)}this._consume(a.tokens.equal,"Expected '=' for let.");const n=this._short_circuit_or_expression();return new te(e,t,null,null,n)}if(this._match(a.keywords.const)){const e=this._consume(a.tokens.ident,"Expected name for const.").toString();let t=null;if(this._match(a.tokens.colon)){const s=this._attribute();t=this._type_decl(),t!=null&&(t.attributes=s)}this._consume(a.tokens.equal,"Expected '=' for const.");const n=this._short_circuit_or_expression();return new _e(e,t,null,null,n)}return null}_increment_decrement_statement(){const e=this._current,t=this._unary_expression();if(t==null)return null;if(!this._check(a.increment_operators))return this._current=e,null;const n=this._consume(a.increment_operators,"Expected increment operator");return new mt(n.type===a.tokens.plus_plus?P.increment:P.decrement,t)}_assignment_statement(){let e=null;if(this._check(a.tokens.brace_right))return null;let t=this._match(a.tokens.underscore);if(t||(e=this._unary_expression()),!t&&e==null)return null;const n=this._consume(a.assignment_operators,"Expected assignment operator."),s=this._short_circuit_or_expression();return new dt(D.parse(n.lexeme),e,s)}_func_call_statement(){if(!this._check(a.tokens.ident))return null;const e=this._current,t=this._consume(a.tokens.ident,"Expected function name."),n=this._argument_expression_list();return n===null?(this._current=e,null):new _t(t.lexeme,n)}_loop_statement(){if(!this._match(a.keywords.loop))return null;this._consume(a.tokens.brace_left,"Expected '{' for loop.");const e=[];let t=this._statement();for(;t!==null;){if(Array.isArray(t))for(let s of t)e.push(s);else e.push(t);t=this._statement()}let n=null;return this._match(a.keywords.continuing)&&(n=this._compound_statement()),this._consume(a.tokens.brace_right,"Expected '}' for loop."),new gt(e,n)}_switch_statement(){if(!this._match(a.keywords.switch))return null;const e=this._optional_paren_expression();this._consume(a.tokens.brace_left,"Expected '{' for switch.");const t=this._switch_body();if(t==null||t.length==0)throw this._error(this._previous(),"Expected 'case' or 'default'.");return this._consume(a.tokens.brace_right,"Expected '}' for switch."),new yt(e,t)}_switch_body(){const e=[];if(this._match(a.keywords.case)){const t=this._case_selectors();this._match(a.tokens.colon),this._consume(a.tokens.brace_left,"Exected '{' for switch case.");const n=this._case_body();this._consume(a.tokens.brace_right,"Exected '}' for switch case."),e.push(new zt(t,n))}if(this._match(a.keywords.default)){this._match(a.tokens.colon),this._consume(a.tokens.brace_left,"Exected '{' for switch default.");const t=this._case_body();this._consume(a.tokens.brace_right,"Exected '}' for switch default."),e.push(new Vt(t))}if(this._check([a.keywords.default,a.keywords.case])){const t=this._switch_body();e.push(t[0])}return e}_case_selectors(){var e,t,n,s;const i=[(t=(e=this._shift_expression())===null||e===void 0?void 0:e.evaluate(this._context).toString())!==null&&t!==void 0?t:""];for(;this._match(a.tokens.comma);)i.push((s=(n=this._shift_expression())===null||n===void 0?void 0:n.evaluate(this._context).toString())!==null&&s!==void 0?s:"");return i}_case_body(){if(this._match(a.keywords.fallthrough))return this._consume(a.tokens.semicolon,"Expected ';'"),[];let e=this._statement();if(e==null)return[];e instanceof Array||(e=[e]);const t=this._case_body();return t.length==0?e:[...e,t[0]]}_if_statement(){if(!this._match(a.keywords.if))return null;const e=this._optional_paren_expression(),t=this._compound_statement();let n=[];this._match_elseif()&&(n=this._elseif_statement(n));let s=null;return this._match(a.keywords.else)&&(s=this._compound_statement()),new xt(e,t,n,s)}_match_elseif(){return this._tokens[this._current].type===a.keywords.else&&this._tokens[this._current+1].type===a.keywords.if?(this._advance(),this._advance(),!0):!1}_elseif_statement(e=[]){const t=this._optional_paren_expression(),n=this._compound_statement();return e.push(new Pt(t,n)),this._match_elseif()&&this._elseif_statement(e),e}_return_statement(){if(!this._match(a.keywords.return))return null;const e=this._short_circuit_or_expression();return new wt(e)}_short_circuit_or_expression(){let e=this._short_circuit_and_expr();for(;this._match(a.tokens.or_or);)e=new S(this._previous().toString(),e,this._short_circuit_and_expr());return e}_short_circuit_and_expr(){let e=this._inclusive_or_expression();for(;this._match(a.tokens.and_and);)e=new S(this._previous().toString(),e,this._inclusive_or_expression());return e}_inclusive_or_expression(){let e=this._exclusive_or_expression();for(;this._match(a.tokens.or);)e=new S(this._previous().toString(),e,this._exclusive_or_expression());return e}_exclusive_or_expression(){let e=this._and_expression();for(;this._match(a.tokens.xor);)e=new S(this._previous().toString(),e,this._and_expression());return e}_and_expression(){let e=this._equality_expression();for(;this._match(a.tokens.and);)e=new S(this._previous().toString(),e,this._equality_expression());return e}_equality_expression(){const e=this._relational_expression();return this._match([a.tokens.equal_equal,a.tokens.not_equal])?new S(this._previous().toString(),e,this._relational_expression()):e}_relational_expression(){let e=this._shift_expression();for(;this._match([a.tokens.less_than,a.tokens.greater_than,a.tokens.less_than_equal,a.tokens.greater_than_equal]);)e=new S(this._previous().toString(),e,this._shift_expression());return e}_shift_expression(){let e=this._additive_expression();for(;this._match([a.tokens.shift_left,a.tokens.shift_right]);)e=new S(this._previous().toString(),e,this._additive_expression());return e}_additive_expression(){let e=this._multiplicative_expression();for(;this._match([a.tokens.plus,a.tokens.minus]);)e=new S(this._previous().toString(),e,this._multiplicative_expression());return e}_multiplicative_expression(){let e=this._unary_expression();for(;this._match([a.tokens.star,a.tokens.forward_slash,a.tokens.modulo]);)e=new S(this._previous().toString(),e,this._unary_expression());return e}_unary_expression(){return this._match([a.tokens.minus,a.tokens.bang,a.tokens.tilde,a.tokens.star,a.tokens.and])?new Et(this._previous().toString(),this._unary_expression()):this._singular_expression()}_singular_expression(){const e=this._primary_expression(),t=this._postfix_expression();return t&&(e.postfix=t),e}_postfix_expression(){if(this._match(a.tokens.bracket_left)){const e=this._short_circuit_or_expression();this._consume(a.tokens.bracket_right,"Expected ']'.");const t=this._postfix_expression();return t&&(e.postfix=t),e}if(this._match(a.tokens.period)){const e=this._consume(a.tokens.ident,"Expected member name."),t=this._postfix_expression(),n=new we(e.lexeme);return t&&(n.postfix=t),n}return null}_getStruct(e){return this._context.aliases.has(e)?this._context.aliases.get(e).type:this._context.structs.has(e)?this._context.structs.get(e):null}_primary_expression(){if(this._match(a.tokens.ident)){const n=this._previous().toString();if(this._check(a.tokens.paren_left)){const s=this._argument_expression_list(),i=this._getStruct(n);return i!=null?new L(i,s):new ve(n,s)}if(this._context.constants.has(n)){const s=this._context.constants.get(n);return new be(n,s.value)}return new ke(n)}if(this._match(a.const_literal))return new Se(parseFloat(this._previous().toString()));if(this._check(a.tokens.paren_left))return this._paren_expression();if(this._match(a.keywords.bitcast)){this._consume(a.tokens.less_than,"Expected '<'.");const n=this._type_decl();this._consume(a.tokens.greater_than,"Expected '>'.");const s=this._paren_expression();return new At(n,s)}const e=this._type_decl(),t=this._argument_expression_list();return new It(e,t)}_argument_expression_list(){if(!this._match(a.tokens.paren_left))return null;const e=[];do{if(this._check(a.tokens.paren_right))break;const t=this._short_circuit_or_expression();e.push(t)}while(this._match(a.tokens.comma));return this._consume(a.tokens.paren_right,"Expected ')' for agument list"),e}_optional_paren_expression(){this._match(a.tokens.paren_left);const e=this._short_circuit_or_expression();return this._match(a.tokens.paren_right),new Te([e])}_paren_expression(){this._consume(a.tokens.paren_left,"Expected '('.");const e=this._short_circuit_or_expression();return this._consume(a.tokens.paren_right,"Expected ')'."),new Te([e])}_struct_decl(){if(!this._match(a.keywords.struct))return null;const e=this._consume(a.tokens.ident,"Expected name for struct.").toString();this._consume(a.tokens.brace_left,"Expected '{' for struct body.");const t=[];for(;!this._check(a.tokens.brace_right);){const s=this._attribute(),i=this._consume(a.tokens.ident,"Expected variable name.").toString();this._consume(a.tokens.colon,"Expected ':' for struct member type.");const c=this._attribute(),h=this._type_decl();h!=null&&(h.attributes=c),this._check(a.tokens.brace_right)?this._match(a.tokens.comma):this._consume(a.tokens.comma,"Expected ',' for struct member."),t.push(new Ft(i,h,s))}this._consume(a.tokens.brace_right,"Expected '}' after struct body.");const n=new z(e,t);return this._context.structs.set(e,n),n}_global_variable_decl(){const e=this._variable_decl();return e&&this._match(a.tokens.equal)&&(e.value=this._const_expression()),e}_override_variable_decl(){const e=this._override_decl();return e&&this._match(a.tokens.equal)&&(e.value=this._const_expression()),e}_global_const_decl(){if(!this._match(a.keywords.const))return null;const e=this._consume(a.tokens.ident,"Expected variable name");let t=null;if(this._match(a.tokens.colon)){const i=this._attribute();t=this._type_decl(),t!=null&&(t.attributes=i)}let n=null;if(this._match(a.tokens.equal)){const i=this._short_circuit_or_expression();if(i instanceof L)n=i;else if(i instanceof be&&i.initializer instanceof L)n=i.initializer;else try{const c=i.evaluate(this._context);n=new Se(c)}catch{n=i}}const s=new _e(e.toString(),t,"","",n);return this._context.constants.set(s.name,s),s}_global_let_decl(){if(!this._match(a.keywords.let))return null;const e=this._consume(a.tokens.ident,"Expected variable name");let t=null;if(this._match(a.tokens.colon)){const s=this._attribute();t=this._type_decl(),t!=null&&(t.attributes=s)}let n=null;return this._match(a.tokens.equal)&&(n=this._const_expression()),new te(e.toString(),t,"","",n)}_const_expression(){if(this._match(a.const_literal))return new we(this._previous().toString());const e=this._type_decl();this._consume(a.tokens.paren_left,"Expected '('.");let t=[];for(;!this._check(a.tokens.paren_right)&&(t.push(this._const_expression()),!!this._check(a.tokens.comma));)this._advance();return this._consume(a.tokens.paren_right,"Expected ')'."),new L(e,t)}_variable_decl(){if(!this._match(a.keywords.var))return null;let e="",t="";this._match(a.tokens.less_than)&&(e=this._consume(a.storage_class,"Expected storage_class.").toString(),this._match(a.tokens.comma)&&(t=this._consume(a.access_mode,"Expected access_mode.").toString()),this._consume(a.tokens.greater_than,"Expected '>'."));const n=this._consume(a.tokens.ident,"Expected variable name");let s=null;if(this._match(a.tokens.colon)){const i=this._attribute();s=this._type_decl(),s!=null&&(s.attributes=i)}return new I(n.toString(),s,e,t,null)}_override_decl(){if(!this._match(a.keywords.override))return null;const e=this._consume(a.tokens.ident,"Expected variable name");let t=null;if(this._match(a.tokens.colon)){const n=this._attribute();t=this._type_decl(),t!=null&&(t.attributes=n)}return new de(e.toString(),t,null)}_enable_directive(){const e=this._consume(a.tokens.ident,"identity expected.");return new vt(e.toString())}_type_alias(){const e=this._consume(a.tokens.ident,"identity expected.");this._consume(a.tokens.equal,"Expected '=' for type alias.");let t=this._type_decl();if(t===null)throw this._error(this._peek(),"Expected Type for Alias.");this._context.aliases.has(t.name)&&(t=this._context.aliases.get(t.name).type);const n=new ge(e.toString(),t);return this._context.aliases.set(n.name,n),n}_type_decl(){if(this._check([a.tokens.ident,...a.texel_format,a.keywords.bool,a.keywords.f32,a.keywords.i32,a.keywords.u32])){const n=this._advance(),s=n.toString();return this._context.structs.has(s)?this._context.structs.get(s):this._context.aliases.has(s)?this._context.aliases.get(s).type:new E(n.toString())}let e=this._texture_sampler_types();if(e)return e;if(this._check(a.template_types)){let n=this._advance().toString(),s=null,i=null;return this._match(a.tokens.less_than)&&(s=this._type_decl(),i=null,this._match(a.tokens.comma)&&(i=this._consume(a.access_mode,"Expected access_mode for pointer").toString()),this._consume(a.tokens.greater_than,"Expected '>' for type.")),new ye(n,s,i)}if(this._match(a.keywords.ptr)){let n=this._previous().toString();this._consume(a.tokens.less_than,"Expected '<' for pointer.");const s=this._consume(a.storage_class,"Expected storage_class for pointer");this._consume(a.tokens.comma,"Expected ',' for pointer.");const i=this._type_decl();let c=null;return this._match(a.tokens.comma)&&(c=this._consume(a.access_mode,"Expected access_mode for pointer").toString()),this._consume(a.tokens.greater_than,"Expected '>' for pointer."),new Tt(n,s.toString(),i,c)}const t=this._attribute();if(this._match(a.keywords.array)){let n=null,s=-1;const i=this._previous();if(this._match(a.tokens.less_than)){n=this._type_decl(),this._context.aliases.has(n.name)&&(n=this._context.aliases.get(n.name).type);let c="";this._match(a.tokens.comma)&&(c=this._shift_expression().evaluate(this._context).toString()),this._consume(a.tokens.greater_than,"Expected '>' for array."),s=c?parseInt(c):0}return new xe(i.toString(),t,n,s)}return null}_texture_sampler_types(){if(this._match(a.sampler_type))return new R(this._previous().toString(),null,null);if(this._match(a.depth_texture_type))return new R(this._previous().toString(),null,null);if(this._match(a.sampled_texture_type)||this._match(a.multisampled_texture_type)){const e=this._previous();this._consume(a.tokens.less_than,"Expected '<' for sampler type.");const t=this._type_decl();return this._consume(a.tokens.greater_than,"Expected '>' for sampler type."),new R(e.toString(),t,null)}if(this._match(a.storage_texture_type)){const e=this._previous();this._consume(a.tokens.less_than,"Expected '<' for sampler type.");const t=this._consume(a.texel_format,"Invalid texel format.").toString();this._consume(a.tokens.comma,"Expected ',' after texel format.");const n=this._consume(a.access_mode,"Expected access mode for storage texture type.").toString();return this._consume(a.tokens.greater_than,"Expected '>' for sampler type."),new R(e.toString(),t,n)}return null}_attribute(){let e=[];for(;this._match(a.tokens.attr);){const t=this._consume(a.attribute_name,"Expected attribute name"),n=new Ee(t.toString(),null);if(this._match(a.tokens.paren_left)){if(n.value=this._consume(a.literal_or_ident,"Expected attribute value").toString(),this._check(a.tokens.comma)){this._advance();do{const s=this._consume(a.literal_or_ident,"Expected attribute value").toString();n.value instanceof Array||(n.value=[n.value]),n.value.push(s)}while(this._match(a.tokens.comma))}this._consume(a.tokens.paren_right,"Expected ')'")}e.push(n)}for(;this._match(a.tokens.attr_left);){if(!this._check(a.tokens.attr_right))do{const t=this._consume(a.attribute_name,"Expected attribute name"),n=new Ee(t.toString(),null);if(this._match(a.tokens.paren_left)){if(n.value=[this._consume(a.literal_or_ident,"Expected attribute value").toString()],this._check(a.tokens.comma)){this._advance();do{const s=this._consume(a.literal_or_ident,"Expected attribute value").toString();n.value.push(s)}while(this._match(a.tokens.comma))}this._consume(a.tokens.paren_right,"Expected ')'")}e.push(n)}while(this._match(a.tokens.comma));this._consume(a.tokens.attr_right,"Expected ']]' after attribute declarations")}return e.length==0?null:e}}class F{constructor(e,t){this.name=e,this.attributes=t,this.size=0}get isArray(){return!1}get isStruct(){return!1}get isTemplate(){return!1}}class Ve{constructor(e,t,n){this.name=e,this.type=t,this.attributes=n,this.offset=0,this.size=0}get isArray(){return this.type.isArray}get isStruct(){return this.type.isStruct}get isTemplate(){return this.type.isTemplate}get align(){return this.type.isStruct?this.type.align:0}get members(){return this.type.isStruct?this.type.members:null}get format(){return this.type.isArray?this.type.format:this.type.isTemplate?this.type.format:null}get count(){return this.type.isArray?this.type.count:0}get stride(){return this.type.isArray?this.type.stride:this.size}}class H extends F{constructor(e,t){super(e,t),this.members=[],this.align=0}get isStruct(){return!0}}class re extends F{constructor(e,t){super(e,t),this.count=0,this.stride=0}get isArray(){return!0}}class Me extends F{constructor(e,t,n,s){super(e,n),this.format=t,this.access=s}get isTemplate(){return!0}}var g;(function(r){r[r.Uniform=0]="Uniform",r[r.Storage=1]="Storage",r[r.Texture=2]="Texture",r[r.Sampler=3]="Sampler",r[r.StorageTexture=4]="StorageTexture"})(g||(g={}));class j{constructor(e,t,n,s,i,c,h){this.name=e,this.type=t,this.group=n,this.binding=s,this.attributes=i,this.resourceType=c,this.access=h}get isArray(){return this.type.isArray}get isStruct(){return this.type.isStruct}get isTemplate(){return this.type.isTemplate}get size(){return this.type.size}get align(){return this.type.isStruct?this.type.align:0}get members(){return this.type.isStruct?this.type.members:null}get format(){return this.type.isArray?this.type.format:this.type.isTemplate?this.type.format:null}get count(){return this.type.isArray?this.type.count:0}get stride(){return this.type.isArray?this.type.stride:this.size}}class Bt{constructor(e,t){this.name=e,this.type=t}}class X{constructor(e,t){this.align=e,this.size=t}}class qt{constructor(e,t,n,s){this.name=e,this.type=t,this.locationType=n,this.location=s,this.interpolation=null}}class Pe{constructor(e,t,n,s){this.name=e,this.type=t,this.locationType=n,this.location=s}}class Nt{constructor(e,t=null){this.stage=null,this.inputs=[],this.outputs=[],this.resources=[],this.name=e,this.stage=t}}class Ot{constructor(){this.vertex=[],this.fragment=[],this.compute=[]}}class Gt{constructor(e,t,n,s){this.name=e,this.type=t,this.attributes=n,this.id=s}}class Dt{constructor(e){this.resources=null,this.node=e}}class A{constructor(e){this.uniforms=[],this.storage=[],this.textures=[],this.samplers=[],this.aliases=[],this.overrides=[],this.structs=[],this.entry=new Ot,this._types=new Map,this._functions=new Map,e&&this.update(e)}_isStorageTexture(e){return e.name=="texture_storage_1d"||e.name=="texture_storage_2d"||e.name=="texture_storage_2d_array"||e.name=="texture_storage_3d"}update(e){const n=new Ct().parse(e);for(const s of n)s instanceof ee&&this._functions.set(s.name,new Dt(s));for(const s of n){if(s instanceof z){const i=this._getTypeInfo(s,null);i instanceof H&&this.structs.push(i);continue}if(s instanceof ge){this.aliases.push(this._getAliasInfo(s));continue}if(s instanceof de){const i=s,c=this._getAttributeNum(i.attributes,"id",0),h=i.type!=null?this._getTypeInfo(i.type,i.attributes):null;this.overrides.push(new Gt(i.name,h,i.attributes,c));continue}if(this._isUniformVar(s)){const i=s,c=this._getAttributeNum(i.attributes,"group",0),h=this._getAttributeNum(i.attributes,"binding",0),f=this._getTypeInfo(i.type,i.attributes),p=new j(i.name,f,c,h,i.attributes,g.Uniform,i.access);this.uniforms.push(p);continue}if(this._isStorageVar(s)){const i=s,c=this._getAttributeNum(i.attributes,"group",0),h=this._getAttributeNum(i.attributes,"binding",0),f=this._getTypeInfo(i.type,i.attributes),p=this._isStorageTexture(f),m=new j(i.name,f,c,h,i.attributes,p?g.StorageTexture:g.Storage,i.access);this.storage.push(m);continue}if(this._isTextureVar(s)){const i=s,c=this._getAttributeNum(i.attributes,"group",0),h=this._getAttributeNum(i.attributes,"binding",0),f=this._getTypeInfo(i.type,i.attributes),p=this._isStorageTexture(f),m=new j(i.name,f,c,h,i.attributes,p?g.StorageTexture:g.Texture,i.access);p?this.storage.push(m):this.textures.push(m);continue}if(this._isSamplerVar(s)){const i=s,c=this._getAttributeNum(i.attributes,"group",0),h=this._getAttributeNum(i.attributes,"binding",0),f=this._getTypeInfo(i.type,i.attributes),p=new j(i.name,f,c,h,i.attributes,g.Sampler,i.access);this.samplers.push(p);continue}if(s instanceof ee){const i=this._getAttribute(s,"vertex"),c=this._getAttribute(s,"fragment"),h=this._getAttribute(s,"compute"),f=i||c||h;if(f){const p=new Nt(s.name,f==null?void 0:f.name);p.inputs=this._getInputs(s.args),p.outputs=this._getOutputs(s.returnType),p.resources=this._findResources(s),this.entry[f.name].push(p)}continue}}}_findResource(e){for(const t of this.uniforms)if(t.name==e)return t;for(const t of this.storage)if(t.name==e)return t;for(const t of this.textures)if(t.name==e)return t;for(const t of this.samplers)if(t.name==e)return t;return null}_findResources(e){const t=[],n=this,s=[];return e.search(i=>{if(i instanceof W)s.push({});else if(i instanceof Y)s.pop();else if(i instanceof I){if(s.length>0){const c=i;s[s.length-1][c.name]=c}}else if(i instanceof te){if(s.length>0){const c=i;s[s.length-1][c.name]=c}}else if(i instanceof ke){const c=i;if(s.length>0&&s[s.length-1][c.name])return;const h=n._findResource(c.name);h&&t.push(h)}else if(i instanceof ve){const c=i,h=n._functions.get(c.name);h&&(h.resources===null&&(h.resources=n._findResources(h.node)),t.push(...h.resources))}}),[...new Map(t.map(i=>[i.name,i])).values()]}getBindGroups(){const e=[];function t(n,s){n>=e.length&&(e.length=n+1),e[n]===void 0&&(e[n]=[]),s>=e[n].length&&(e[n].length=s+1)}for(const n of this.uniforms){t(n.group,n.binding);const s=e[n.group];s[n.binding]=n}for(const n of this.storage){t(n.group,n.binding);const s=e[n.group];s[n.binding]=n}for(const n of this.textures){t(n.group,n.binding);const s=e[n.group];s[n.binding]=n}for(const n of this.samplers){t(n.group,n.binding);const s=e[n.group];s[n.binding]=n}return e}_getOutputs(e,t=void 0){if(t===void 0&&(t=[]),e instanceof z)this._getStructOutputs(e,t);else{const n=this._getOutputInfo(e);n!==null&&t.push(n)}return t}_getStructOutputs(e,t){for(const n of e.members)if(n.type instanceof z)this._getStructOutputs(n.type,t);else{const s=this._getAttribute(n,"location")||this._getAttribute(n,"builtin");if(s!==null){const i=this._getTypeInfo(n.type,n.type.attributes),c=this._parseInt(s.value),h=new Pe(n.name,i,s.name,c);t.push(h)}}}_getOutputInfo(e){const t=this._getAttribute(e,"location")||this._getAttribute(e,"builtin");if(t!==null){const n=this._getTypeInfo(e,e.attributes),s=this._parseInt(t.value);return new Pe("",n,t.name,s)}return null}_getInputs(e,t=void 0){t===void 0&&(t=[]);for(const n of e)if(n.type instanceof z)this._getStructInputs(n.type,t);else{const s=this._getInputInfo(n);s!==null&&t.push(s)}return t}_getStructInputs(e,t){for(const n of e.members)if(n.type instanceof z)this._getStructInputs(n.type,t);else{const s=this._getInputInfo(n);s!==null&&t.push(s)}}_getInputInfo(e){const t=this._getAttribute(e,"location")||this._getAttribute(e,"builtin");if(t!==null){const n=this._getAttribute(e,"interpolation"),s=this._getTypeInfo(e.type,e.attributes),i=this._parseInt(t.value),c=new qt(e.name,s,t.name,i);return n!==null&&(c.interpolation=this._parseString(n.value)),c}return null}_parseString(e){return e instanceof Array&&(e=e[0]),e}_parseInt(e){e instanceof Array&&(e=e[0]);const t=parseInt(e);return isNaN(t)?e:t}_getAlias(e){for(const t of this.aliases)if(t.name==e)return t.type;return null}_getAliasInfo(e){return new Bt(e.name,this._getTypeInfo(e.type,null))}_getTypeInfo(e,t){if(this._types.has(e))return this._types.get(e);if(e instanceof xe){const s=e,i=this._getTypeInfo(s.format,s.attributes),c=new re(s.name,t);return c.format=i,c.count=s.count,this._types.set(e,c),this._updateTypeInfo(c),c}if(e instanceof z){const s=e,i=new H(s.name,t);for(const c of s.members){const h=this._getTypeInfo(c.type,c.attributes);i.members.push(new Ve(c.name,h,c.attributes))}return this._types.set(e,i),this._updateTypeInfo(i),i}if(e instanceof R){const s=e,i=s.format instanceof E,c=s.format?i?this._getTypeInfo(s.format,null):new F(s.format,null):null,h=new Me(s.name,c,t,s.access);return this._types.set(e,h),this._updateTypeInfo(h),h}if(e instanceof ye){const s=e,i=s.format?this._getTypeInfo(s.format,null):null,c=new Me(s.name,i,t,s.access);return this._types.set(e,c),this._updateTypeInfo(c),c}const n=new F(e.name,t);return this._types.set(e,n),this._updateTypeInfo(n),n}_updateTypeInfo(e){var t,n;const s=this._getTypeSize(e);if(e.size=(t=s==null?void 0:s.size)!==null&&t!==void 0?t:0,e instanceof re){const i=this._getTypeSize(e.format);e.stride=(n=i==null?void 0:i.size)!==null&&n!==void 0?n:0,this._updateTypeInfo(e.format)}e instanceof H&&this._updateStructInfo(e)}_updateStructInfo(e){var t;let n=0,s=0,i=0,c=0;for(let h=0,f=e.members.length;h<f;++h){const p=e.members[h],m=this._getTypeSize(p);if(!m)continue;(t=this._getAlias(p.type.name))!==null&&t!==void 0||p.type;const x=m.align,O=m.size;n=this._roundUp(x,n+s),s=O,i=n,c=Math.max(c,x),p.offset=n,p.size=O,this._updateTypeInfo(p.type)}e.size=this._roundUp(c,i+s),e.align=c}_getTypeSize(e){var t;if(e==null)return null;const n=this._getAttributeNum(e.attributes,"size",0),s=this._getAttributeNum(e.attributes,"align",0);if(e instanceof Ve&&(e=e.type),e instanceof F){const i=this._getAlias(e.name);i!==null&&(e=i)}{const i=A._typeInfo[e.name];if(i!==void 0){const c=e.format==="f16"?2:1;return new X(Math.max(s,i.align/c),Math.max(n,i.size/c))}}{const i=A._typeInfo[e.name.substring(0,e.name.length-1)];if(i){const c=e.name[e.name.length-1]==="h"?2:1;return new X(Math.max(s,i.align/c),Math.max(n,i.size/c))}}if(e instanceof re){let i=e,c=8,h=8;const f=this._getTypeSize(i.format);f!==null&&(h=f.size,c=f.align);const p=i.count,m=this._getAttributeNum((t=e==null?void 0:e.attributes)!==null&&t!==void 0?t:null,"stride",this._roundUp(c,h));return h=p*m,n&&(h=n),new X(Math.max(s,c),Math.max(n,h))}if(e instanceof H){let i=0,c=0,h=0,f=0,p=0;for(const m of e.members){const x=this._getTypeSize(m.type);x!==null&&(i=Math.max(x.align,i),h=this._roundUp(x.align,h+f),f=x.size,p=h)}return c=this._roundUp(i,p+f),new X(Math.max(s,i),Math.max(n,c))}return null}_isUniformVar(e){return e instanceof I&&e.storage=="uniform"}_isStorageVar(e){return e instanceof I&&e.storage=="storage"}_isTextureVar(e){return e instanceof I&&e.type!==null&&A._textureTypes.indexOf(e.type.name)!=-1}_isSamplerVar(e){return e instanceof I&&e.type!==null&&A._samplerTypes.indexOf(e.type.name)!=-1}_getAttribute(e,t){const n=e;if(!n||!n.attributes)return null;const s=n.attributes;for(let i of s)if(i.name==t)return i;return null}_getAttributeNum(e,t,n){if(e===null)return n;for(let s of e)if(s.name==t){let i=s!==null&&s.value!==null?s.value:n;return i instanceof Array&&(i=i[0]),typeof i=="number"?i:typeof i=="string"?parseInt(i):n}return n}_roundUp(e,t){return Math.ceil(t/e)*e}}A._typeInfo={f16:{align:2,size:2},i32:{align:4,size:4},u32:{align:4,size:4},f32:{align:4,size:4},atomic:{align:4,size:4},vec2:{align:8,size:8},vec3:{align:16,size:12},vec4:{align:16,size:16},mat2x2:{align:8,size:16},mat3x2:{align:8,size:24},mat4x2:{align:8,size:32},mat2x3:{align:16,size:32},mat3x3:{align:16,size:48},mat4x3:{align:16,size:64},mat2x4:{align:16,size:32},mat3x4:{align:16,size:48},mat4x4:{align:16,size:64}},A._textureTypes=a.any_texture_type.map(r=>r.name),A._samplerTypes=a.sampler_type.map(r=>r.name);function U(r,e){return Object.fromEntries(e.map(t=>{const n=jt(r,t,0);return[t.name,{typeDefinition:n,group:t.group,binding:t.binding,size:n.size}]}))}function Fe(r,e,t){return{fields:Object.fromEntries(e.members.map(s=>[s.name,{offset:s.offset,type:ae(r,s.type,0)}])),size:e.size,offset:t}}function Rt(r){var e;if(r.name.includes("depth"))return"depth";switch((e=r.format)==null?void 0:e.name){case"f32":return"float";case"i32":return"sint";case"u32":return"uint";default:throw new Error("unknown texture sample type")}}function Ue(r){return r.name.includes("2d_array")?"2d-array":r.name.includes("cube_array")?"cube-array":r.name.includes("3d")?"3d":r.name.includes("1d")?"1d":r.name.includes("cube")?"cube":"2d"}function Lt(r){switch(r.access){case"read":return"read-only";case"write":return"write-only";case"read_write":return"read-write";default:throw new Error("unknonw storage texture access")}}function Wt(r){return r.name.endsWith("_comparison")?"comparison":"filtering"}function Yt(r,e){const{binding:t,access:n,type:s}=r;switch(r.resourceType){case g.Uniform:return{binding:t,visibility:e,buffer:{}};case g.Storage:return{binding:t,visibility:e,buffer:{type:n===""||n==="read"?"read-only-storage":"storage"}};case g.Texture:{if(s.name==="texture_external")return{binding:t,visibility:e,externalTexture:{}};const i=s.name.includes("multisampled");return{binding:t,visibility:e,texture:{sampleType:Rt(s),viewDimension:Ue(s),multisampled:i}}}case g.Sampler:return{binding:t,visibility:e,sampler:{type:Wt(s)}};case g.StorageTexture:return{binding:t,visibility:e,storageTexture:{access:Lt(s),format:s.format.name,viewDimension:Ue(s)}};default:throw new Error("unknown resource type")}}function ne(r,e){const t={};for(const n of r)t[n.name]={stage:e,resources:n.resources.map(s=>{const{name:i,group:c}=s;return{name:i,group:c,entry:Yt(s,e)}})};return t}function Ht(r){const e=new A(r),t=Object.fromEntries(e.structs.map(m=>[m.name,Fe(e,m,0)])),n=U(e,e.uniforms),s=U(e,e.storage.filter(m=>m.resourceType===g.Storage)),i=U(e,e.storage.filter(m=>m.resourceType===g.StorageTexture)),c=U(e,e.textures.filter(m=>m.type.name!=="texture_external")),h=U(e,e.textures.filter(m=>m.type.name==="texture_external")),f=U(e,e.samplers),p={...ne(e.entry.vertex,GPUShaderStage.VERTEX),...ne(e.entry.fragment,GPUShaderStage.FRAGMENT),...ne(e.entry.compute,GPUShaderStage.COMPUTE)};return{externalTextures:h,samplers:f,structs:t,storages:s,storageTextures:i,textures:c,uniforms:n,entryPoints:p}}function se(r,e=""){if(!r)throw new Error(e)}function jt(r,e,t){switch(e.resourceType){case g.Uniform:case g.Storage:case g.StorageTexture:return ae(r,e.type,t);default:return{size:0,type:e.type.name}}}function ae(r,e,t){if(e.isArray){se(!e.isStruct,"struct array is invalid"),se(!e.isStruct,"template array is invalid");const n=e;return{size:n.size,elementType:ae(r,n.format,t),numElements:n.count}}else{if(e.isStruct)return se(!e.isTemplate,"template struct is invalid"),Fe(r,e,t);{const n=e,s=e.isTemplate?`${n.name}<${n.format.name}>`:e.name;return{size:e.size,type:s}}}}const Xt=new Map([[Int8Array,{formats:["sint8","snorm8"],defaultForType:1}],[Uint8Array,{formats:["uint8","unorm8"],defaultForType:1}],[Int16Array,{formats:["sint16","snorm16"],defaultForType:1}],[Uint16Array,{formats:["uint16","unorm16"],defaultForType:1}],[Int32Array,{formats:["sint32","snorm32"],defaultForType:0}],[Uint32Array,{formats:["uint32","unorm32"],defaultForType:0}],[Float32Array,{formats:["float32","float32"],defaultForType:0}]]);new Map([...Xt.entries()].map(([r,{formats:[e,t]}])=>[[e,r],[t,r]]).flat());const Kt=r=>r.createBindGroupLayout({entries:[{binding:0,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:1,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:2,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:3,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:4,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}},{binding:5,visibility:GPUShaderStage.COMPUTE,buffer:{type:"uniform"}}]});function $t(r){const e=Ht(le),t=pe(e.uniforms.flames),n=Kt(r),s=r.createBuffer({size:4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),i=r.createBuffer({size:4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),c=r.createBuffer({size:4*3,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),h=r.createBuffer({size:4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),f=r.createBuffer({size:t.arrayBuffer.byteLength,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),p=r.createBuffer({size:4,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});return{bindgroup:r.createBindGroup({layout:n,entries:[{binding:0,resource:{buffer:s}},{binding:1,resource:{buffer:i}},{binding:2,resource:{buffer:c}},{binding:3,resource:{buffer:h}},{binding:4,resource:{buffer:f}},{binding:5,resource:{buffer:p}}]}),bindgroupLayout:n,flamesVariableDefinition:e.uniforms.flames,buffers:{gamma:s,logDensity:i,densityEstimation:c,antialiasing:h,flames:f,timeElapsed:p}}}function Ce(r){return{enabled:[r.enabled],color:{r:[r.color.r],g:[r.color.g],b:[r.color.b]},colorPaletteIdx:[r.colorPaletteIndex],weight:[r.weight],transform:{a:[r.transform.a],b:[r.transform.b],c:[r.transform.c],d:[r.transform.d],e:[r.transform.e],f:[r.transform.f]},variations:r.weightedVariations.map(e=>({weight:[e.weight],variation:[Ye(e.variation)]}))}}function Jt(r,e,t,n){var i,c,h,f;const s=pe(n.flamesVariableDefinition);s.set({resolution:[1920,1080],palette:{a:[e.namedPalette.palette.a.x,e.namedPalette.palette.a.y,e.namedPalette.palette.a.z],b:[e.namedPalette.palette.b.x,e.namedPalette.palette.b.y,e.namedPalette.palette.b.z],c:[e.namedPalette.palette.c.x,e.namedPalette.palette.c.y,e.namedPalette.palette.c.z],d:[e.namedPalette.palette.d.x,e.namedPalette.palette.d.y,e.namedPalette.palette.d.z]},gammaCorrection:e.gammaCorrection,densityEstimation:{enabled:[e.densityEstimation!=null],minsigma:[((i=e.densityEstimation)==null?void 0:i.minSigma)??0],maxsigma:[((c=e.densityEstimation)==null?void 0:c.maxSigma)??0]},antialiasing:[e.antialiasing?1:0],renderMode:[Xe(e.renderMode)],spaceWarp:{zoom:[e.spaceWarp.zoom],rotationalSymmetry:[e.spaceWarp.rotationalSymmetry],mirrorX:[e.spaceWarp.mirrorX],mirrorY:[e.spaceWarp.mirrorY]},finalComponent:Ce(e.final),components:e.components.map(Ce)}),r.queue.writeBuffer(n.buffers.flames,0,s.arrayBuffer),r.queue.writeBuffer(n.buffers.timeElapsed,0,new Uint32Array([Date.now()])),r.queue.writeBuffer(t.buffers.heatmapMax,0,new Uint32Array([0])),r.queue.writeBuffer(n.buffers.gamma,0,new Float32Array([e.gammaCorrection])),r.queue.writeBuffer(n.buffers.logDensity,0,new Float32Array([+(e.renderMode!=He)])),r.queue.writeBuffer(n.buffers.densityEstimation,0,new Float32Array([e!=null&&e.densityEstimation?1:0,((h=e==null?void 0:e.densityEstimation)==null?void 0:h.minSigma)??0,((f=e==null?void 0:e.densityEstimation)==null?void 0:f.maxSigma)??0])),r.queue.writeBuffer(n.buffers.antialiasing,0,new Float32Array([e.antialiasing?1:0]))}function C(r,e,t){return r.createComputePipeline({layout:e,compute:{module:r.createShaderModule({code:t}),entryPoint:"main"}})}let w,ie={x:0,y:0},Be,B,q,K,y,qe,Ne,Oe,Ge,De,V,$;function N(r,e,t=1920,n=1080){r.setPipeline(e),r.setBindGroup(0,q.bindgroup),r.setBindGroup(1,K.bindgroup),r.dispatchWorkgroups(t/8,n/8)}async function Zt(r){Jt(y,w,q,K);let e=q.buffers.finalImage,t=y.createCommandEncoder({label:"Compute encoder"}),n=t.beginComputePass();N(n,Ne,16,16),N(n,Oe),w!=null&&w.antialiasing?N(n,De):N(n,Ge),w!=null&&w.densityEstimation&&(N(n,qe),e=q.buffers.blurredImage),n.end(),t.copyBufferToBuffer(e,0,B,0,B.size),y.queue.submit([t.finish()]),await B.mapAsync(GPUMapMode.READ);const s=new Uint8ClampedArray(B.getMappedRange()),i=new ImageData(s,1920,1080);r.putImageData(i,0,0),B.unmap()}function Re(){let r=y.createCommandEncoder({label:"Reset render data encoder"}),e=r.beginComputePass();N(e,$),e.end(),y.queue.submit([r.finish()])}async function Qt(r){!Be||!w||await Zt(r)}async function er(r,e){const t=e.getContext("2d");if(t===null){console.error("Failure to initialize the flames worker due to invaldie canvas context");return}y=await(await navigator.gpu.requestAdapter()).requestDevice(),q=tt(y),K=$t(y),B=y.createBuffer({size:1920*1080*4,usage:GPUBufferUsage.MAP_READ|GPUBufferUsage.COPY_DST}),V=y.createPipelineLayout({bindGroupLayouts:[q.bindgroupLayout,K.bindgroupLayout]}),Ne=C(y,V,Ze),Oe=C(y,V,$e),De=C(y,V,Ke),Ge=C(y,V,Je),qe=C(y,V,le),$=C(y,V,Qe),ie={x:e.width,y:e.height},w=r,Be??(Be=new Uint8ClampedArray(ie.x*ie.y*4)),requestAnimationFrame(s);async function s(){w!==void 0&&t&&await Qt(t),setTimeout(()=>requestAnimationFrame(s),1e3/60)}}function tr(r){w=r,$&&Re()}function rr(r){w=r,$&&Re()}function nr(r){w=r}onmessage=async({data:r})=>{const e=je(r.rawFlames);switch(r.resetType){case"init":r.canvasContext&&await er(e,r.canvasContext);break;case"full":tr(e);break;case"soft":rr(e);break;case"none":nr(e);break}postMessage({flames:JSON.stringify(e,null,4)})}})();
